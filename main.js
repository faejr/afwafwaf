afwaf = (function() {
    function encrypt(text) {
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            var bitArray = new BitArray(8, ShiftLeft(character & 0xFF, ++num % 8));
            bitArray = bitArray.reverse();
            for (var j = 0; j < bitArray.getLength(); j += 2) {
                var x = bitArray.getAt(j);
                var y = bitArray.getAt(j + 1);
                     if (x && y) { result += 'a'; }
                else if (x && !y) { result += 'w'; }
                else if (!x && y) { result += 'f'; }
                else if (!x && !y) { result += 'g'; }
            }
        }
        return result;
    }
    ...
    function decrypt(text) {
        ...
        for (var i = 0; i < text.length; i += 4) {
            ...
            for (var j = 0; j < 4; j++) {
                ...
                var y = false;
                switch (c) {
                case 'a': x = true; y = true; break;
                case 'w': x = true; y = false; break;
                case 'f': x = false; y = true; break;
                case 'g': x = false; y = false; break;
                }
                bitArray.setAt(j * 2, x);
                bitArray.setAt(j * 2 + 1, y);
            }
            bitArray = bitArray.reverse();
            result += String.fromCharCode(ShiftRight(parseInt(bitArray, 2), i / 4 % 8));
        }
        return result;
    }

    function ShiftLeft(b, dist) {
        var result;
        if ((dist & 7) == 0) {
            result = b;
        } else {
        ...
        if (size && this.m_bits.length != size) {
            if (this.m_bits.length < size) {
                for (var i = this.m_bits.length; i < size; i++) {
                    this.m_bits.unshift(BitArray._OFF); // Add to beginning of array, mimicking C#
                }
            } else {
    ...
    BitArray.prototype.reverse = function () { m_bits = this.m_bits.reverse(); return this; }
